...
Sando comes with 8 handcrafted flavors. Switch your entire UI's personality by setting the `flavor` attribute:

```html
<!-- Default flavor (original) — warm orange tones -->
<sando-button variant="solid">Original</sando-button>

<!-- Neutral flavor — cool slate grays + blue (B2B/enterprise) -->
<sando-button variant="solid" flavor="neutral">Neutral</sando-button>

<!-- Strawberry flavor — pink/red tones (opt-in showcase) -->
...
```html
<!-- Everything inside inherits the neutral flavor -->
<div flavor="neutral">
  <sando-button variant="solid">I'm neutral</sando-button>
  <sando-input label="Me too" placeholder="Professional tone"></sando-input>
  <sando-checkbox label="Same here"></sando-checkbox>
</div>
```

**Core flavors** — always bundled with `@sando-ds/tokens/css`:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>`neutral`</td>
      <td>Professional B2B — cool slate</td>
      <td>Blue</td>
    </tr>
    <tr>
      <td>`sando`</td>
...
**Showcase flavors** — opt-in via `import "@sando-ds/tokens/css/flavors/<name>"`:
