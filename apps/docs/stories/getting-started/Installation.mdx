import { Meta } from "@storybook/blocks";

<Meta title="Getting Started/Installation" />

# Installation

You're just a few commands away from adding Sando to your project. Let's get your kitchen set up.

## Prerequisites

Before you start, make sure you have:

- **Node.js** ≥ 20.0.0
- **A modern browser** — Chrome/Edge 90+, Firefox 88+, Safari 14+, Opera 76+
- **A package manager** — pnpm (recommended), npm, or yarn

> **💡 Why pnpm?**
>
> We recommend [pnpm](https://pnpm.io/) for faster installs and efficient disk usage. Sando itself is built with pnpm. But npm and yarn work perfectly fine too.

## Install Packages

Sando has two packages — **tokens** (the design language) and **components** (the UI elements). Install both:

```bash
# pnpm
pnpm add @sando-ds/tokens @sando-ds/components

# npm
npm install @sando-ds/tokens @sando-ds/components

# yarn
yarn add @sando-ds/tokens @sando-ds/components
```

<details>
<summary>Only need tokens?</summary>

If you're building your own components and just want the design tokens:

```bash
pnpm add @sando-ds/tokens
```

</details>

## What's in the Box

### `@sando-ds/tokens`

The design language — your pantry of raw ingredients, flavor profiles, and component recipes:

- **CSS custom properties** — ready to use in any framework
- **Three-layer architecture** — Ingredients → Flavors → Recipes
- **8 curated flavors** — Core 3 (original, neutral, sando) bundled by default; Showcase 5 (tonkatsu, strawberry, egg-salad, kiwi, nori) available as opt-in imports
- **22 OKLCH color palettes** — 17 chromatic + 4 neutral + 1 utility, perceptually uniform, accessible by default
- **Automatic accessibility modes** — dark, high contrast, reduced motion, forced colors

### `@sando-ds/components`

Framework-agnostic Web Components built with [Lit](https://lit.dev):

- **25+ production components** — button, input, checkbox, switch, select, textarea, and more
- **14 skeleton loading patterns** — for polished loading states
- **TypeScript definitions** included
- **Tree-shakeable** ES modules — import only what you need

## Import Tokens

Load the design tokens CSS in your application entry point:

```ts
// Vite / Webpack — main.ts or App.tsx
import "@sando-ds/tokens/css";
```

```html
<!-- HTML -->
<link
  rel="stylesheet"
  href="node_modules/@sando-ds/tokens/dist/sando-tokens/css/index.css"
/>
```

```css
/* CSS — styles.css */
@import "@sando-ds/tokens/css";
```

## Import Components

Import components individually — they self-register as custom elements:

```ts
// Import the components you need
import "@sando-ds/components/button";

// That's it! Use them in your HTML
// <sando-button variant="solid">Click me</sando-button>
```

## Load a Flavor

Flavors are loaded globally so they can cascade into component Shadow DOMs. Import a flavor CSS file to activate a theme:

```ts
// Load a flavor (global, Light DOM)
import "@sando-ds/tokens/css/flavors/original/flavor.css";

// Or try a different one
import "@sando-ds/tokens/css/flavors/tonkatsu/flavor.css";
import "@sando-ds/tokens/css/flavors/strawberry/flavor.css";
```

> **💡 Flavors inherit automatically**
>
> CSS custom properties from flavors naturally inherit from the Light DOM into Shadow DOM. No extra configuration needed — just import the flavor CSS globally.

## Framework-Specific Setup

### React / Next.js

```tsx
// App.tsx
import "@sando-ds/tokens/css";
import "@sando-ds/components/button";

function App() {
  return <sando-button variant="solid">Click me</sando-button>;
}
```

> **💡 TypeScript Support**
>
> Add to your `vite-env.d.ts` or `global.d.ts`:
>
> ```ts
> /// <reference types="@sando-ds/components" />
> ```
>
> This enables autocomplete and type checking for Sando components in JSX.

### Vue 3

```vue
<script setup lang="ts">
import "@sando-ds/tokens/css";
import "@sando-ds/components/button";
</script>

<template>
  <sando-button variant="solid">Click me</sando-button>
</template>
```

> **💡 Vue Config**
>
> Tell Vue to treat `sando-*` as custom elements in `vite.config.ts`:
>
> ```ts
> export default defineConfig({
>   plugins: [
>     vue({
>       template: {
>         compilerOptions: {
>           isCustomElement: (tag) => tag.startsWith("sando-"),
>         },
>       },
>     }),
>   ],
> });
> ```

### Angular

```ts
// app.module.ts
import { CUSTOM_ELEMENTS_SCHEMA } from "@angular/core";
import "@sando-ds/tokens/css";

@NgModule({
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class AppModule {}
```

```ts
// component.ts
import "@sando-ds/components/button";
```

```html
<!-- component.html -->
<sando-button variant="solid">Click me</sando-button>
```

### Svelte

```svelte
<script>
  import '@sando-ds/tokens/css';
  import '@sando-ds/components/button';
</script>

<sando-button variant="solid">Click me</sando-button>
```

### Vanilla JavaScript

```html
<!doctype html>
<html lang="en">
  <head>
    <link
      rel="stylesheet"
      href="node_modules/@sando-ds/tokens/dist/sando-tokens/css/index.css"
    />
    <script type="module">
      import "@sando-ds/components/button";
    </script>
  </head>
  <body>
    <sando-button variant="solid">Click me</sando-button>
  </body>
</html>
```

## Selective Imports

Import only what you need for smaller bundles:

```ts
// Import specific components
import "@sando-ds/components/button"; // <sando-button>
// import '@sando-ds/components/input';   // <sando-input> — not yet exported individually
// import '@sando-ds/components/checkbox'; // <sando-checkbox> — not yet exported individually

// Import specific token layers
import "@sando-ds/tokens/css/ingredients"; // Primitives only
import "@sando-ds/tokens/css/flavors"; // Semantic tokens
import "@sando-ds/tokens/css/recipes"; // Component tokens
```

## CDN Usage

For quick prototyping or static sites:

```html
<!-- Load CSS tokens -->
<link
  rel="stylesheet"
  href="https://unpkg.com/@sando-ds/tokens/dist/sando-tokens/css/index.css"
/>

<!-- Load components -->
<script type="module">
  import "@sando-ds/components/button";
</script>
```

> **⚠️ Production Usage**
>
> For production applications, install via a package manager for better performance, caching, and version control.

## Verify Your Installation

Paste this into your HTML to confirm everything is wired up:

```html
<script type="module">
  import "@sando-ds/components/button";
</script>

<sando-button variant="solid">It works! 🍱</sando-button>
```

If you see a styled button, your Sando kitchen is ready.

## Browser Support

<table>
  <thead>
    <tr>
      <th>Browser</th>
      <th>Minimum Version</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>Chrome/Edge</td>
      <td>90+</td>
    </tr>
    <tr>
      <td>Firefox</td>
      <td>88+</td>
    </tr>
    <tr>
      <td>Safari</td>
      <td>14+</td>
    </tr>
    <tr>
      <td>Opera</td>
      <td>76+</td>
    </tr>
  </tbody>
</table>

## Next Steps

- **[Quick Start →](?path=/docs/getting-started-quick-start--docs)** — Build your first component in 5 minutes
- **[Theming Guide →](?path=/docs/getting-started-theming--docs)** — Learn flavors, dark mode, and customization
- **[Token Architecture →](?path=/docs/guides-architecture--docs)** — Understand the three-layer system
