...
### 🎨 8 Handcrafted Flavors

Each flavor is a complete brand identity, hand-tuned for harmony across light, dark, and high-contrast modes. Flavors are split into **Core** (bundled by default) and **Showcase** (opt-in imports):

**Core** — always included with `@sando-ds/tokens/css`:
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>`neutral`</td>
      <td>Professional baseline</td>
      <td>Cool slate + blue</td>
    </tr>
    <tr>
      <td>`sando`</td>
...
**Showcase** — opt-in via `import "@sando-ds/tokens/css/flavors/<name>"`:
