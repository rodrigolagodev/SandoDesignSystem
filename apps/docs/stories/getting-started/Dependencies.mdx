...
> **💡 Why pnpm?**
>
> The Sando monorepo uses pnpm workspaces for efficient dependency management. While you can use npm or yarn to install `@sando-ds/tokens` and `@sando-ds/components` in your own project, pnpm is required if you're contributing to the design system itself.
...
The components package depends on Lit for Web Components rendering and on `@sando-ds/tokens` for design tokens:

```json
{
  "dependencies": {
    "lit": "^3.1.0",
    "@sando-ds/tokens": "workspace:*"
  }
}
```
...
import "@sando-ds/tokens/css/ingredients/color";
import "@sando-ds/tokens/css/flavors/original";
```
...
> **💡 Selective Imports**
>
> Currently, only `./button` has a dedicated export path. All other components are available through the main `@sando-ds/components` entry point. More individual exports will be added as the library matures.
