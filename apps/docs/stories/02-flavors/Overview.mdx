...
Think of Flavors like the seasoning in a sandwich—they take raw ingredients and give them purpose and personality. In Sando's three-layer architecture:

```
🥓 Ingredients (Primitives)
   ↓ referenced by
🥬 Flavors (Semantic) ← YOU ARE HERE
   ↓ referenced by
...
```css
/* ❌ BAD: Using Ingredients directly in components */
.button {
  background: var(--sando-color-orange-700);
  color: var(--sando-color-neutralWarm-100);
}

/* ✅ GOOD: Using Flavors for semantic meaning */
.button {
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Red</td>
      <td>Vibrant, fruity, energetic</td>
    </tr>
    <tr>
      <td>
    ...
    <tr>
      ...
      <td>Green</td>
      <td>Fresh, natural, growth-focused</td>
    </tr>
    <tr>
      <td>
        <strong>egg-salad</strong>
      </td>
      <td>Yellow</td>
      <td>Bright, cheerful, inviting</td>
    </tr>
  </tbody>
</table>
...
You don't need to write any mode-switching logic—it's automatic!
...
<div flavor="tonkatsu">
  <sando-button>Auto Dark Mode (Tonkatsu)</sando-button>
</div>
```
...
1. Change `--sando-color-orange-700` → updates all orange UI elements
2. Change `--sando-color-action-solid-background-default` → rethemes all solid actions
3. Change `--sando-button-solid-backgroundColor-default` → rethemes all solid buttons
...
- [Recipes](/?path=/docs/flavors-recipes--docs) - Component-specific tokens

---

**Flavors are the key to effortless theming in Sando. Master them, and you'll unlock unlimited design possibilities! 🎨**
