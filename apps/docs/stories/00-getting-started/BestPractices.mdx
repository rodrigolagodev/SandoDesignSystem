...
├─ YES → Use Recipe tokens
│   Example: --sando-button-solid-backgroundColor-default
│
└─ NO → Are you defining semantic roles?
    ├─ YES → Use Flavor tokens
    │   Example: --sando-color-action-solid-background-default
    │
    └─ NO → Use Ingredient tokens (primitives)
        Example: --sando-color-orange-700
```

### ✅ DO: Use the Right Layer

```css
/* ✅ GOOD: Component using Recipe tokens */
.custom-card {
...
/* ✅ GOOD: Theme customization using Flavor tokens */
[flavor="custom"] {
...
/* ✅ GOOD: Defining new Flavor tokens from Ingredients */
:root {
...
### ❌ DON'T: Skip Layers or Mix Incorrectly

```css
/* ❌ BAD: Component using Ingredient tokens directly */
.custom-card {
  background: var(--sando-color-orange-700); /* Skip Recipe AND Flavor layers */
  color: var(--sando-color-neutralWarm-100);
}

/* ❌ BAD: Recipe tokens pointing to Ingredients (skip Flavor) */
:root {
  --sando-button-custom-backgroundColor: var(--sando-color-blue-600); /* Should use Flavor */
}
...
/* ❌ Don't use primitive colors for UI */
.card {
...
- [ ] **Touch Targets** - Minimum 44×44px for touch devices
...
```html
<!-- ✅ GOOD: Proper label and error message -->
<sando-input
  label="Email"
  required
...
<!-- ❌ BAD: Missing label, no error association -->
<sando-input
  placeholder="Email"
  required
></sando-input>
...
```typescript
// ✅ GOOD: Import specific components (smaller bundle)
import '@sando/components/button';
import '@sando/components/input';

// ❌ BAD: Import all components (larger bundle)
...
| Package | Budget (gzipped) | Actual |
|---------|------------------|--------|
| @sando/tokens | < 5 KB | ~3 KB |
| @sando/components (each) | < 10 KB | ~6-8 KB |
...
<!-- ❌ BAD: Reinventing components from scratch -->
...
```html
<!-- ✅ GOOD: Apply flavor at container level -->
<div flavor="ocean">
...
<!-- ❌ BAD: Apply flavor to each component -->
<div>
...
```html
<!-- ✅ GOOD: Variants for visual styles -->
...
```css
/* ❌ BAD: Fighting with Shadow DOM */
sando-button {
  background: red !important; /* Won't work! */
}

/* ✅ GOOD: Use CSS custom properties (design tokens) */
sando-button {
  --sando-button-solid-backgroundColor-default: red;
}

/* ✅ BETTER: Create custom flavor */
[flavor="danger"] {
  --sando-color-action-solid-background-default: var(--sando-color-state-error-500);
}
```
...
**Following these practices will help you build accessible, performant, and maintainable UIs with Sando! 🚀**
