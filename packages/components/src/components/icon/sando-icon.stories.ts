	...
	render: () => html`
		<div style="display: flex; gap: 2rem; align-items: center; flex-wrap: wrap;">
			...
			<div style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem;">
				...
				<span style="font-size: 0.75rem; color: var(--sando-color-text-caption);">small (14px)</span>
			</div>
			<div style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem;">
				<sando-icon name="star" size="medium"></sando-icon>
				<span style="font-size: 0.75rem; color: var(--sando-color-text-caption);">medium (18px)</span>
			</div>
			<div style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem;">
				<sando-icon name="star" size="large"></sando-icon>
				<span style="font-size: 0.75rem; color: var(--sando-color-text-caption);">large (24px)</span>
			</div>
			<div style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem;">
...
export const Transformations: Story = {
	render: () => html`
		<div style="display: flex; flex-direction: column; gap: 2rem;">
			<!-- Rotation -->
			<div>
				<h4 style="margin-bottom: 1rem; font-size: 0.875rem; color: var(--sando-color-text-caption);">Rotation</h4>
			...
			<!-- Flipping -->
			<div>
				<h4 style="margin-bottom: 1rem; font-size: 0.875rem; color: var(--sando-color-text-caption);">Flipping</h4>
				<div style="display: flex; gap: 2rem; align-items: center;">
					<div style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem;">
					...
					<div style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem;">
						...
						<span style="font-size: 0.75rem; color: var(--sando-color-text-caption);">flip-horizontal</span>
					</div>
					<div style="display: flex; flex-direction: column; align-items: center; gap: 0.5rem;">
						<sando-icon name="arrow-down" size="large" flip-vertical></sando-icon>
						<span style="font-size: 0.75rem; color: var(--sando-color-text-caption);">flip-vertical</span>
					</div>
				</div>
			</div>
		</div>
...
export const WithText: Story = {
	render: () => html`
		<div style="display: flex; flex-direction: column; gap: 1.5rem;">
			...
			<div>
				<h4 style="margin-top: 1rem; margin-bottom: 0.5rem; font-size: 0.875rem; color: var(--sando-color-text-caption);">
	...
	render: () => {
		...
		const categories: Record<string, string[]> = {
			...
			Media: iconNames.filter((name) =>
				['play', 'pause', 'music', 'video', 'image', 'camera', 'film'].some((term) => name.includes(term))
			),
			Files: iconNames.filter((name) =>
				['file', 'folder', 'document', 'upload', 'download'].some((term) => name.includes(term))
			),
			Social: iconNames.filter((name) =>
				['twitter', 'facebook', 'instagram', 'github', 'linkedin', 'youtube'].some((term) => name.includes(term))
			),
		...
		return html`
			<div style="display: flex; flex-direction: column; gap: 3rem;">
				<div style="padding: 1rem; background: var(--sando-color-background-surface); border-radius: 8px;">
					...
					([category, icons]) => html`
						<div>
							<h3 style="margin-bottom: 1rem; color: var(--sando-color-text-heading);">
								${category} <span style="color: var(--sando-color-text-caption); font-weight: normal;">(${icons.length})</span>
							</h3>
							<div
							...
							>
								...
								${icons.length > 100
									? html`
											<div style="grid-column: 1 / -1; text-align: center; padding: 1rem; color: var(--sando-color-text-caption);">
		...
		<div style="display: flex; flex-direction: column; gap: 2rem;">
			<div>
				...
				<div style="display: flex; gap: 2rem; flex-wrap: wrap;">
					<button style="display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem 1rem; border: 1px solid #ccc; border-radius: 4px; background: white; cursor: pointer;">
			...
			<div style="padding: 1rem; background: var(--sando-color-background-surface); border-radius: 8px;">
				<p style="margin: 0; font-size: 0.875rem; color: var(--sando-color-text-body);">
					<strong>Best Practice:</strong> Use <code>aria-label</code> when the icon conveys information, use
					<code>decorative</code> when it's purely visual enhancement.
				</p>
			</div>
		</div>
	`,
