# Prompt Template: Content Creation

Use this template for writing, documentation, marketing copy, or any content creation.
...
ROLE: You are a {{ROLE/EXPERTISE}} with {{EXPERIENCE_LEVEL}}.
...
ACTION: Write {{COPY_TYPE}} for {{PLACEMENT}}
FORMAT: {{SPECS}} (character limits, structure)
TARGET: {{AUDIENCE}} with {{PAIN_POINTS}}
```
...
ACTION: Create {{DOC_TYPE}} (tutorial, reference, guide)
FORMAT: {{STRUCTURE}} with code examples
TARGET: {{USER_LEVEL}} (beginner, intermediate, expert)
```
...
ACTION: Write {{EMAIL_TYPE}} (welcome, nurture, promotional)
FORMAT: Subject line + preview + body + CTA
TARGET: {{SUBSCRIBER_SEGMENT}}
```
